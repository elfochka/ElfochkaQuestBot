...
def read_story():
    try:
        df = pd.read_excel("./games/zombie_land/zombie_land_story.xlsx")
...
def start_game(step, tg_user_id, user_choice=None):
    ...
    try:
        ...
        if step not in story:
            ...
            markup = generate_game_markup(None, "qbgm_zombie_")
            return text, markup
        ...
        markup = generate_game_markup(actions, "qbgm_zombie_")  # передаем префикс
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('qbgm_zombie_'))
def callback_inline(call):
    try:
        if call.message:
            with bot.retrieve_data(call.message.chat.id, call.message.chat.id) as data:
                game_id = data['game_id']
            step = call.data.replace('qbgm_zombie_', '')  # Убираем префикс
            if step == 'refresh':
                step = 0
            else:
                step = int(step)
